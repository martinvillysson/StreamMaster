...
export function GetDropDownLine({
	...props
}: DropDownLineProps): React.ReactElement {
	const { currentSetting, updateStateAndRequest, updateSettingRequest } =
		useSettingsContext();
	...
	const getValue = useMemo(() => {
		if (propertyExists(updateSettingRequest.Parameters, props.field)) {
			return (
				getRecordString<UpdateSettingParameters>(
					props.field,
		...
		return (
			getRecordString<UpdateSettingParameters>(props.field, currentSetting) ??
			""
		);
	...
	const buttonTemplate = (): ReactNode => {
		var found = props.options.find(
			(o) => o.label?.toLowerCase() === getValue.toLowerCase(),
		);
		// if (!found) {
