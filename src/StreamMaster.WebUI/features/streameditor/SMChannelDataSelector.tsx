...
import AutoSetEPGSMChannelButton from "@components/smchannels/AutoSetEPGSMChannelButton";
...
const SMChannelDataSelector = ({
	enableEdit: propsEnableEdit,
	id,
}: SMChannelDataSelectorProperties) => {
	...
	const { columnConfig: channelNumberColumnConfig } =
		useSMChannelNumberColumnConfig({ enableEdit, useFilter: false });
	const { columnConfig: channelLogoColumnConfig } =
		useSMChannelLogoColumnConfig({ enableEdit });
	const channelNameColumnConfig = useSMChannelNameColumnConfig({
		width: smTableIsSimple ? 200 : 125,
	});
	const epgColumnConfig = useSMChannelEPGColumnConfig({
		width: smTableIsSimple ? 150 : 125,
	});
	...
	const { columnConfig: proxyColumnConfig } =
		useSMCommandProfileNameColumnConfig({ enableEdit, useFilter: false });
	...
	const channelDelete = useCallback(
		(smChannel: SMChannelDto) => {
			if (smDataTableRef.current) {
				if (
					selectedSMChannel === undefined ||
					selectedSMChannel?.Id === smChannel.Id
				) {
