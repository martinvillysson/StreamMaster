...
const SMStreamDataSelector = ({
	enableEdit: propsEnableEdit,
	height,
	id,
	simple = false,
	showSelections,
}: SMStreamDataSelectorProperties) => {
	...
	const smStreamMembershipColumnConfig = useSMStreamMembershipColumnConfig({
		dataKey,
	});
	...
	const simpleColumns = useMemo(
		(): ColumnMeta[] => [
			{ field: "Name", filter: true, sortable: true, width: "10rem" },
		],
		[],
	);
	...
	const addOrRemoveTemplate = useCallback(
		(data: any) => {
			...
			const found =
				selectedSMChannel?.SMStreamDtos?.some(
					(item) => item.Id === smStream.Id,
				) ?? false;
						...
						<div className="flex align-content-center justify-content-center">
							<SMButton
								...
								onClick={() => {
									...
									const request: RemoveSMStreamFromSMChannelRequest = {
				...
				return (
					<div className="flex align-content-center justify-content-center">
						<SMButton
							...
							onClick={() => {
								const request = {
									SMChannelId: selectedSMChannel?.Id ?? 0,
									SMStreamId: smStream.Id,
								} as AddSMStreamToSMChannelRequest;
			...
			return (
				<div className="flex align-content-center justify-content-center">
					<SMButton
						...
						onClick={() => {
							const request = {
								StreamIds: [smStream.Id],
							} as CreateSMChannelsFromStreamsRequest;
	...
	const rowClass = useCallback(
		(data: unknown): string => {
			...
			if (
				data &&
				selectedSMChannel?.SMStreamDtos !== undefined &&
				selectedSMChannel.SMStreamDtos.length > 0
			) {
	...
	return (
		<SMDataTable
			...
			onClick={(e: any) => {
				if (
					e.target.className &&
					e.target.className === "p-datatable-wrapper"
				) {
					setSelectedSMChannel(undefined);
				}
			}}
