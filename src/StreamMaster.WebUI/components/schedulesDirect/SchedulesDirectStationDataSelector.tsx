...
import type { ColumnMeta } from "@components/smDataTable/types/ColumnMeta";
...
const SchedulesDirectStationDataSelector = () => {
	...
	const { selectedItems, setSelectedItems } =
		useSelectedItems<StationPreview>(dataKey);
	const [originalSelectedItems, setOriginalSelectedItems] = useState<
		StationPreview[] | undefined
	>(undefined);
	const isInitialLoad = useRef(true);

	useEffect(() => {
		if (
			(originalSelectedItems !== undefined &&
				originalSelectedItems.length > 0) ||
		...
		const sp = schedulesDirectGetSelectedStationIdsQuery.data
			.map((stationIdLineUp) =>
				stationPreviews.data?.find(
					(stationPreview) =>
						stationPreview.StationId === stationIdLineUp.StationId &&
						stationPreview.Lineup === stationIdLineUp.Lineup,
				),
			)
	...
	const onSave = useCallback(
		(stationIdLineUps: StationPreview[]) => {
			...
			if (originalSelectedItems !== undefined) {
				if (
					arraysEqualByKey(originalSelectedItems, selectedItems, "StationId")
				) {
					return;
				}
			}
...
SchedulesDirectStationDataSelector.displayName =
