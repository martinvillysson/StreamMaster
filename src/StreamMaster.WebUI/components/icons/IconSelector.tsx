		...
		if (large) {
			return (
				<div className="flex justify-content-center align-items-center w-full">
					<img
						className="icon-template-lg dark-background"
						alt="Icon logo"
						src={iconUrl}
					/>
				</div>
			...
			<div className="w-full flex flex-row align-items-center justify-content-between p-row-odd">
				<div className="flex sm-w-4rem icon-selector-template">
					<img
						...
						alt={icon.Name}
						onError={(e) =>
							((e.currentTarget as HTMLImageElement).src = SMLogo)
						}
						loading="lazy"
					/>
				</div>
				<div className="w-9">
					<div
						className={`${icon.Name.length > 30 ? "sm-text-xs" : "sm-text-lg"} sm-w-1rem`}
					>
						{icon.Name}
					</div>
				</div>
