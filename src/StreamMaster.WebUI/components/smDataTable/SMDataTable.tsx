...
import {
	...
	getHeaderFromField,
	setColumnToggle,
} from "./helpers/dataSelectorFunctions";
...
import {
	SMDataTableProps,
	SMDataTableRef,
} from "./types/smDataTableInterfaces";
...
const SMDataTable = <T extends DataTableValue>(
	props: SMDataTableProps<T>,
	ref: React.Ref<SMDataTableRef<T>>,
) => {
	const { state, setters } = useSMDataSelectorValuesState<T>(
		props.id,
	...
	const { queryFilter } = useSetQueryFilter(
		props.id,
		props.columns,
		state.first,
		state.filters,
	...
	const { data, isLoading } = props.queryFilter
		? props.queryFilter(queryFilter)
		...
		(options: ColumnFilterElementTemplateOptions) => {
			...
			if (col.filterElement) {
				if (col.sortable === true) {
					return (
						<div
							className={`flex flex-row ${justify} max-w-full flex-grow-0 overflow-hidden`}
						>
							<div className="max-w-full w-full flex-grow-0 overflow-hidden text-overflow-ellipsis">
								{col.filterElement(options)}
							</div>
							{col.sortable === true && (
								<div
									className="max-w-full flex-grow-0 overflow-hidden text-overflow-ellipsis"
									style={{ paddingLeft: "0.16rem", width: "1.84rem" }}
								>
									{sortButton(options)}
								</div>
							)}
				...
				return (
					<div
						className={`flex flex-row ${justify} max-w-full flex-grow-0 overflow-hidden`}
					>
						<div className="max-w-full flex-grow-0 overflow-hidden text-overflow-ellipsis">
							{col.filterElement(options)}
						</div>
					</div>
				);
			...
			if (col.fieldType === "actions") {
				if (props.actionHeaderTemplate) {
					return (
						<div
							className={`flex ${justify} align-items-center align-content-center`}
						>
				...
				if (
					...
					state.visibleColumns !== undefined
				) {
					selectedCols = state.visibleColumns.filter(
						(col) => col.removable === true && col.removed !== true,
					);

					return (
						<MultiSelect
							className="multiselectcolumn w-full input-height-with-no-borders"
							value={selectedCols === null ? "" : selectedCols}
							options={cols}
							optionLabel="field"
				...
				return (
					<div
						className={`flex ${justify} align-items-center align-content-center input-height-with-no-borders`}
					>
					...
					if (filter.value && typeof itemValue === "string") {
						if (Array.isArray(filter.value)) {
							return filter.value.some(
								(val) => val.toLowerCase() === itemValue.toLowerCase(),
							);
						}
		...
		(data: T[]) => {
			...
			return dataCopy.sort((a, b) => {
				...
				if (typeof aValue === "number" && typeof bValue === "number") {
					return (
						(aValue < bValue ? -1 : aValue > bValue ? 1 : 0) * state.sortOrder
					);
				}
	...
	const removeSelection = useCallback(
		(data: T) => {
			const newSelectedItems = state.selectedItems.filter(
				(predicate) => predicate.Id !== data.Id,
			);
			if (state.selectAll) {
	...
	function selectionHeaderTemplate() {
		return (
			<div className="flex justify-content-center align-items-center">
				{props.showHiddenInSelection && (
					<SMTriSelectShowHidden dataKey={props.id} />
				)}
				{props.showSelectAll === true && (
					<SMTriSelectShowSelect
				...
				{props.showSelected === true && (
					<SMTriSelectShowSelected dataKey={props.id} />
				)}
	...
	const isLazy = useMemo(() => {
		...
		const la =
			props.lazy === true ||
			(props.queryFilter === undefined ? undefined : true);
	...
	const getRowExpanderHeader = useMemo(() => {
		return (
			<div className="flex align-items-center justify-content-center">
				...
				<div
					className="custom-target-icon pi pi-sort"
					data-pr-position="right"
					data-pr-tooltip="Drag rows to reorder priority"
				/>
			</div>
		);
	}, []);
	...
	function findMissingKeys<T>(
		obj1: Record<string, T> | undefined,
		obj2: Record<string, T> | undefined,
	): Record<string, T> {
		...
		<div
			...
			onClick={(event: any) => {
				...
				if (
					...
					props.rowExpansionTemplate !== undefined
				) {
			...
			<div className={getClass}>
				...
				<Suspense fallback={<div>Loading DataTable...</div>}>
					<LazyDataTable
						...
						first={
							state.pagedInformation
								? state.pagedInformation.First
								: state.first
						}
						loading={
							props.noIsLoading !== true
								? props.isLoading === true || isLoading === true
								: false
						}
						lazy={isLazy}
						onRowCollapse={(e) => {
						...
						onRowToggle={(e: any) => {
							if (props.singleExpand === true) {
								const expandedRows = findMissingKeys(
									state.expandedRows,
									e.data,
								);
					...
					>
						...
						<Column
							...
							hidden={
								!props.showExpand || props.rowExpansionTemplate === undefined
							}
							showFilterMenu={false}
							showFilterOperator={false}
							resizeable={false}
						...
						{props.columns
							...
							.map((col) => (
								<Column
									...
									filter
									filterElement={
										col.headerTemplate ? col.headerTemplate : colFilterTemplate
									}
									filterPlaceholder={
										col.filter === true
											? col.fieldType === "epg"
									...
									field={col.field}
									hidden={
										col.isHidden === true || col.fieldType === "filterOnly"
									}
